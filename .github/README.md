...
docker run -it --rm \
  ...
  -v "$(pwd):/app" \
  -v "$HOME/.ssh:/root/.ssh" \
  ghcr.io/spotdemo4/bumper:0.8.0
```
...
#### [bumper-0.8.0.tar.xz](https://github.com/spotdemo4/nix-scan/releases/latest/download/bumper-0.8.0.tar.xz) - bundle

contains all dependencies, only use if necessary

```elm
wget https://github.com/spotdemo4/nix-scan/releases/latest/download/bumper-0.8.0.tar.xz &&
tar xf bumper-0.8.0.tar.xz &&
./release
```
